#ifndef SIMDJSON_SKYLAKE_X_STAGE1_FIND_MARKS_H
#define SIMDJSON_SKYLAKE_X_STAGE1_FIND_MARKS_H

#include "simdjson/portability.h"

#ifdef IS_X86_64

#include "skylake_x/bitmask.h"
...
really_inline void find_whitespace_and_operators(simd8x64<uint8_t> in, uint64_t &whitespace, uint64_t &op) {
  ...
  whitespace = in.map([&](simd8<uint8_t> _in) {
    return _in == simd8<uint8_t>(_mm512_shuffle_epi8(whitespace_table, _in));
  }).to_bitmask();

  op = in.map([&](simd8<uint8_t> _in) {
    // | 32 handles the fact that { } and [ ] are exactly 32 bytes apart
    return (_in | 32) == simd8<uint8_t>(_mm512_shuffle_epi8(op_table, _in-','));
  }).to_bitmask();
}
...
WARN_UNUSED error_code implementation::stage1(const uint8_t *buf, size_t len, document::parser &parser, bool streaming) const noexcept {
  return skylake_x::stage1::find_structural_bits<128>(buf, len, parser, streaming);
}
...
#endif // IS_X86_64
#endif // SIMDJSON_SKYLAKE_X_STAGE1_FIND_MARKS_H
